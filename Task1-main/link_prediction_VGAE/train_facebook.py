...
# 生成特征矩阵（只使用训练边）
G = nx.from_scipy_sparse_array(adj_label)
node2vec = Node2Vec(G, dimensions=args.input_dim, walk_length=80, num_walks=20, workers=8)
model_embedding = node2vec.fit(window=10, min_count=1, batch_words=4)
feature_mat = np.array([model_embedding.wv[str(node)] for node in range(num_nodes)])
