...
class Net(torch.nn.Module):
    def __init__(self, dim=32):
        ...
        self.conv1 = GCNConv(features_scaled.shape[1], 32)  # 输入特征维度为features_scaled的列数
        self.conv2 = GCNConv(32, 17)  # 输出类别为17（根据你的标签数）
...
for epoch in range(50):
    ...
    accL.append(acc)
    if (epoch+1) % 10 == 0:
        print(f"Epoch {epoch+1}, Loss: {loss}, Test Accuracy: {acc}")

# 输出最佳准确率及其对应的epoch
print(f'Best Test Accuracy: {max(accL)}, Epoch: {accL.index(max(accL))}')
